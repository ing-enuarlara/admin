...
while ($f <= $numFilas) {
	...
	if ($todoBien) {
		...
		if (!empty($idProducto)) {
			...
			if (!empty($fileName)) {
				$sql .= "('" . $idProducto . "', '" . $fileName . "', '" . $arrayIndividual['cpf_tipo'] . "', '" . $principal . "', {$idEmpresa}),";
...
echo '<script type="text/javascript">window.location.href="../modules/comercial/bd_read/productos-importar-fotos.php.php?success=SC_11&' . $summary . '";</script>';
exit();