...
if($extension == 'xlsx'){
	...
	if (move_uploaded_file($temName, $nombreArchivo)) {		
		...
		if ($_FILES['plantilla']['error'] === UPLOAD_ERR_OK){
			...
			$sql = "INSERT INTO comercial_productos_techspecs(cpt_id_producto, cpt_lebel, cpt_value, cpt_id_empresa, cpt_tech_prin) VALUES";
				...
				*/
				...
				if($todoBien) {
					...
					if ($productoExistenteKey !== false) {
						...
						$principal = 1;
						$validarProducto[$datosProductoExistente['cprin_id']] = $arrayIndividual['cpt_id_producto'];
					}

					if(!empty($idProducto)){
						$arrayTodos[$f] = $arrayIndividual;
						$sql .= "('".$idProducto."', '".$arrayIndividual['cpt_lebel']."', '".$arrayIndividual['cpt_value']."', {$idEmpresa}, 'SI'),";
			...
			echo '<script type="text/javascript">window.location.href="../modules/comercial/bd_read/catalogo-principal-importar-especificacioines.php?success=SC_11&'.$summary.'";</script>';
			exit();
			...
			echo '<script type="text/javascript">window.location.href="../modules/comercial/bd_read/catalogo-principal-importar-especificacioines.php?error=ER_6&msj='.$message.'";</script>';
			exit();
		}
	}else{
