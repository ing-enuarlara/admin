                ...
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="ofertas-agregar.php" class="btn btn-primary"><i class="fas fa-solid fa-plus"></i> Agregar Ofertas</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    $filtroAdmin = "";
                                    if ($_SESSION["datosUsuarioActual"]['usr_tipo'] != DEV) {
                                        $filtroAdmin .= " AND ofer_id_empresa='" . $_SESSION["idEmpresa"] . "'";
                                    }
                                    try {
                                        $ofertas = $conexionBdComercial->query("SELECT ofer.*, cliAdmi.cliAdmi_nombre, count(cop_id_articulo) AS numArticulos FROM comercial_ofertas ofer
                                        LEFT JOIN comercial_ofertas_productos ON cop_id_oferta = ofer_id 
                                        INNER JOIN " . BDADMIN . ".clientes_admin cliAdmi ON cliAdmi_id = ofer_id_empresa
                                        WHERE 1=1 {$filtroAdmin}
                                        GROUP BY ofer_id");
                                    } catch (Exception $e) {
                                        include(RUTA_PROYECTO . "includes/error-catch-to-report.php");
                                    }
                                    while ($result = mysqli_fetch_array($ofertas, MYSQLI_BOTH)) {

                                        $estado = "SI";
                                        $color = "green";
                                        if ($result['ofer_activo'] != SI) {
                                            $estado = "NO";
                                            $color = "red";
                                        }

                                        $rutaFoto = "";
                                        if (!empty($result['ofer_img'])) {
                                            switch ($result['ofer_tipo_img']) {
                                                case TIPO_IMG:
                                                    if (file_exists(RUTA_PROYECTO . "files/ofertas/" . $result['ofer_img'])) {
                                                        $rutaFoto = REDIRECT_ROUTE . "files/ofertas/" . $result['ofer_img'];
                                                    }
                                                    break;
                                                case TIPO_URL:
                                                    $rutaFoto = $result['ofer_img'];
                                                    break;
                                            }
                                        }

                                        switch ($result['ofer_tipo']) {
                                            case PROD:
                                                $tipoOferta = "PRODUCTOS";
                                                $colorOferta = "RED";
                                                break;
                                            case CATE:
                                                $tipoOferta = "CATEGORIAS";
                                                $colorOferta = "blue";
                                                break;
                                            case SUB_CATE:
                                                $tipoOferta = "SUB-CATEGORIAS";
                                                $colorOferta = "yellow";
                                                break;
                                            case TIPO:
                                                $tipoOferta = "TIPOS DE PRODUCTOS";
                                                $colorOferta = "orange";
                                                break;

                                            default:
                                                $tipoOferta = "TODA LA TIENDA";
                                                $colorOferta = "green";
                                                break;
                                        }

                                        $texto = !empty($result['ofer_descripcion']) ? htmlspecialchars(strip_tags($result['ofer_descripcion'])) : "";
                                    ?>
                                        <tr>
                                            <td style="text-align: right;"><?= $result['ofer_id']; ?></td>
                                            <td align="center">
                                                <?php if (!empty($rutaFoto)) { ?>
                                                    <img src="<?= $rutaFoto ?>" width="40">
                                                <?php } ?>
                                            </td>
                                            <td><?= date("d M, Y", strtotime($result['ofer_fecha_inicio'])); ?></td>
                                            <td><?= date("d M, Y", strtotime($result['ofer_fecha_fin'])); ?></td>
                                            <td><?= $result['ofer_title']; ?></td>
                                            <td title="<?= $texto; ?>"
                                            ...
                                            <td style="color: <?= $colorOferta; ?>;"><?= $tipoOferta; ?></td>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu" role="menu">
                                                        <a class="dropdown-item" href="ofertas-editar.php?id=<?= $result[0]; ?>" data-toggle="tooltip">Editar</a>
                                                        <a class="dropdown-item" href="../bd_delete/ofertas-eliminar.php?id=<?= $result[0]; ?>" onClick="if(!confirm('Este registro se eliminará del sistema, Desea continuar bajo su responsabilidad?')){return false;}" data-toggle="tooltip">Eliminar</a>
                                                        <!-- <a class="dropdown-item" href="productos-fotos.php?id=<?= $result[0]; ?>" data-toggle="tooltip">Producto</a> -->
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
