                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-success">
                            ...
                            <div class="col-md-12">
                                    <!-- form start -->
                                <form class="form-horizontal" method="post" action="../bd_update/marcas-catalogo-actualizar.php" enctype="multipart/form-data">
                                    <input type="hidden" name="id" value="<?=$_GET["id"];?>">
                                        <div class="form-group col-md-6">
                                            <label for="exampleInputEmail1">Nombre:</label>
                                            <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Nombre de la Sub-Categorias" name="nombre" value="<?=$resultadoD['cmarp_nombre'];?>">
                                        </div>
                                        <div class="form-group col-md-6">
                                            ...
                                            <select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="categoria">
                                                ...
                                                if ($_SESSION["datosUsuarioActual"]['usr_tipo'] != DEV) {
                                                    $predicado = ["ccatp_id_empresa" => $_SESSION["idEmpresa"]];
                                                }
                                                try{
                                                  $categorias = Categorias_Catalogo_Principal::Select($predicado)->fetchAll(PDO::FETCH_ASSOC);
                                                } catch (Exception $e) {
                                                  include(RUTA_PROYECTO."includes/error-catch-to-report.php");
                                                }
                                                foreach ($categorias as $resOp){
                                                  $nombreEmpresa='';
                                                  if($_SESSION["datosUsuarioActual"]['usr_tipo']==DEV){
                                                      try{
                                                        $nomEmpresa = Clientes_Admin::Select([
                                                            "cliAdmi_id" => $resOp['ccatp_id_empresa']
                                                        ])->fetch(PDO::FETCH_ASSOC);
                                                      } catch (Exception $e) {
                                                        include(RUTA_PROYECTO."includes/error-catch-to-report.php");
                                                      }
                                                    ...
                                                    $selected='';
                                                    if($resultadoD['cmarp_categoria']==$resOp['ccatp_id']){$selected='selected';}
                                                ?>
                                                    <option value="<?=$resOp['ccatp_id'];?>" <?=$selected;?>><?=$resOp['ccatp_nombre'].$nombreEmpresa;?></option>
                                                <?php }?>
                                            </select>
                                        </div>
                                        <div class="form-group col-md-6" style="display:block;" id="groupMenu">
                                            ...
                                            <select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="menu" id="menu" onchange="ocultarMasJoyas(this)">
                                              <option value=""></option>
                                              <option value="1"<?php if($resultadoD['cmarp_menu']==1){echo "selected";}?>>SI</option>
                                              <option value="0"<?php if($resultadoD['cmarp_menu']==0){echo "selected";}?>>NO</option>
                                            </select>
                                        </div>
                                        <div class="form-group col-md-6" style="display:block;" id="groupMasJoyas">
                                            ...
                                            <select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="masJoyas" id="masJoyas">
                                              <option value=""></option>
                                              <option value="1"<?php if($resultadoD['cmarp_mas_joyas']==1){echo "selected";}?>>SI</option>
                                              <option value="0"<?php if($resultadoD['cmarp_mas_joyas']==0){echo "selected";}?>>NO</option>
                                            </select>
                                        </div>
                                    </div>
                                    <!-- /.card-body -->
