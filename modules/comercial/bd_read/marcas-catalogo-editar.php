									...
									<form class="form-horizontal" method="post" action="../bd_update/marcas-catalogo-actualizar.php" enctype="multipart/form-data">
										...
										<div class="form-group col-md-6">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" multiple="multiple" style="width: 100%;" name="categorias[]">
												...
												$categorias = Categorias_Catalogo_Principal::Select($predicadoCat)->fetchAll(PDO::FETCH_ASSOC);
												foreach ($categorias as $resOp) {

													$nombreEmpresa = '';
													if ($_SESSION["datosUsuarioActual"]['usr_tipo'] == DEV) {
														$empresa = Clientes_Admin::Select([ 'cliAdmi_id' => $resOp['ccatp_id_empresa'] ])->fetch(PDO::FETCH_ASSOC);
														$nombreEmpresa = "[" . $empresa['cliAdmi_nombre'] . "]";
													}
													$selected = Sub_Categorias::numRows(
														[
															'subca_marca' => $resultadoD['cmarp_id'],
															'subca_cate' => $resOp['ccatp_id'],
															'subca_prin' => SI
														]) ? "selected" : "";
												?>
										...
										<div class="form-group col-md-6" style="display:block;" id="groupMenu">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="menu" id="menu" onchange="ocultarMasJoyas(this)">
												...
												<option value="0" <?php if ($resultadoD['cmarp_menu'] == 0) {
																		echo "selected";
																	} ?>>NO</option>
											</select>
										</div>
										<div class="form-group col-md-6" style="display:block;" id="groupMasJoyas">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="masJoyas" id="masJoyas">
												<option value=""></option>
												<option value="1" <?php if ($resultadoD['cmarp_mas_productos'] == 1) {
																		echo "selected";
																	} ?>>SI</option>
												<option value="0" <?php if ($resultadoD['cmarp_mas_productos'] == 0) {
																		echo "selected";
																	} ?>>NO</option>
											</select>
										</div>
