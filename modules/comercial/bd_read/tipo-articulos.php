                    ...
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                $predicado = [];
                                if ($_SESSION["datosUsuarioActual"]['usr_tipo'] != DEV) {
                                    $predicado = ["ctipop_id_empresa" => $_SESSION["idEmpresa"]];
                                }

                                if (!empty($filtro)) {
                                    $filtro = preg_replace('/\sAND$/', '', $filtro);

                                    $predicado = !empty($predicado) ? array_merge($predicado, [Tipos_Catalogo_Principal::OTHER_PREDICATE => $filtro]) : [Tipos_Catalogo_Principal::OTHER_PREDICATE => $filtro];
                                }

                                Clientes_Admin::foreignKey(Clientes_Admin::INNER, [
                                    "cliAdmi_id" => 'ctipop_id_empresa'
                                ]);
                                try{
                                    $tipoProd = Tipos_Catalogo_Principal::SelectJoin(
                                        $predicado,
                                        "*",
                                        [
                                ...
                                $num=1;
                                foreach ($tipoProd as $result){

                                    $estado="Activo";
                                    $color="green";
                                    if($result['ctipop_estado']!=1){
                                        $estado="Inactivo";
                                        $color="red";
                                    }
                                ?>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                <a class="dropdown-item" href="tipo-articulos-editar.php?id=<?=$result['ctipop_id'];?>" data-toggle="tooltip">Editar</a>
                                                <!--<div class="dropdown-divider"></div>-->
                                                <a class="dropdown-item" href="../bd_delete/tipo-articulos-eliminar.php?id=<?=$result['ctipop_id'];?>" onClick="if(!confirm('Este registro se eliminará del sistema, Desea continuar bajo su responsabilidad?')){return false;}" data-toggle="tooltip">Eliminar</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
