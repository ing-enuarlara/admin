                ...
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="combos-agregar.php" class="btn btn-primary"><i class="fas fa-solid fa-plus"></i> Agregar Combos</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    $predicado = [];
                                    if ($_SESSION["datosUsuarioActual"]['usr_tipo'] != DEV) {
                                        $predicado = [ "combo_empresa" => $_SESSION["idEmpresa"] ];
                                    }
                                    Combos_Productos::foreignKey(Combos_Productos::LEFT, [
                                        "ccp_combo" => "combo_id"
                                    ]);
                                    Clientes_Admin::foreignKey(Clientes_Admin::INNER, [
                                        "cliAdmi_id" => "combo_empresa"
                                    ]);
                                    $combos = Combos::SelectJoin(
                                        $predicado,
                                        "combo.*, cliAdmi.cliAdmi_nombre, count(ccp_producto) AS numProductos",
                                        [
                                            Combos_Productos::class,
                                            Clientes_Admin::class
                                        ], "", "combo_id"
                                    );
                                    foreach ($combos AS $result) {

                                        $estado = "SI";
                                        $color = "green";
                                        if ($result['combo_activo'] != SI) {
                                            $estado = "NO";
                                            $color = "red";
                                        }

                                        $texto = !empty($result['combo_descripcion']) ? htmlspecialchars(strip_tags($result['combo_descripcion'])) : "";
                                    ?>
                                        <tr>
                                            <td style="text-align: right;"><?= $result['combo_id']; ?></td>
                                            <td><?= $result['combo_title']; ?></td>
                                            <td title="<?= $texto; ?>"
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu" role="menu">
                                                        <a class="dropdown-item" href="combos-editar.php?id=<?= $result['combo_id']; ?>" data-toggle="tooltip">Editar</a>
                                                        <a class="dropdown-item" href="../bd_delete/combos-eliminar.php?id=<?= $result['combo_id']; ?>" onClick="if(!confirm('Este registro se eliminará del sistema, Desea continuar bajo su responsabilidad?')){return false;}" data-toggle="tooltip">Eliminar</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
