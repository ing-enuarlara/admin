...
try{
    $consuluta= $conexionBdComercial->query("SELECT * FROM comercial_remisiones INNER JOIN comercial_clientes ON cli_id=remi_cliente WHERE remi_id='".$_GET["id"]."'");
} catch (Exception $e) {
...
<script type="text/javascript">
    function clientes(datoscliente){
        ...
        location.href = "remisiones-editar.php?id=<?=$_GET["id"];?>&cte="+id+"&fact=<?=$factura;?>#productos";
    }
    ...
    function getPay(pay){
        ...
        if(pay.value == 1){
            msg = 'La remisión cambiará a pesos colombianos. Es decir que se tomará el valor en dolares y se multiplicará por el TRM de venta actual.';
        }else if(pay.value == 2){
            msg = 'La remisión cambiará a dólares americanos. Es decir que se tomará el valor en pesos y se dividirá entre el TRM de compra actual.';
        }
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                    ...
                    <p>
                        <a href="remisiones-agregar.php" class="btn btn-primary"><i class="fas fa-solid fa-plus"></i> Agregar nuevo</a>
                        <a href="../../reportes/formato-remision-1.php?id=<?=$_GET["id"];?>" class="btn btn-success" target="_blank"><i class="icon-print"></i> Imprimir</a>
                        <?php
                        if($factura!=1){
                        ?>
                            <a href="../bd_create/remisiones-generar-factura.php?id=<?= $resultadoD['remi_id']; ?>" class="btn btn-info" onClick="if(!confirm('Desea generar la factura de esta remisión?')){return false;}"><i class="icon-money"></i> Generar Factura</a>
                        <?php
                        }
                        ?>
                    </p>
                    ...
                    ?>
                        ...
                        <p style="color: black; background-color: gold; padding: 10px; font-weight: bold;"> No es posible hacer más cambios en esta remisión.</p>
                    <?php
                    }
                    ?>	
                    <div class="row">
                                ...
                                <form class="form-horizontal" method="post" action="../bd_update/remisiones-actualizar.php" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body">
                                            ...
                                            <select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="vendedor" required <?=$disabled?>>
                                                ...
                                                <?php
                                                    ...
                                                    while($result = mysqli_fetch_array($consulta, MYSQLI_BOTH)){
                                                        ...
                                                        $selected='';
                                                        if($resultadoD['remi_vendedor']==$result[0]){
                                                            $selected = 'selected';
                                                        }
                                                ?>
                                        ...
                                        <div class="form-group col-md-2">
                                            ...
                                            <input type="date" class="form-control" id="exampleInputEmail1" name="fechaPropuesta" required value="<?=$resultadoD['remi_fecha_propuesta'];?>" <?=$disabled?>>
                                        </div>
                                        
                                        <div class="form-group col-md-2">
                                            <label for="exampleInputEmail1">Fecha de vencimiento:</label>
                                            <input type="date" class="form-control" id="exampleInputEmail1" name="fechaVencimiento" required value="<?=$resultadoD['remi_fecha_vencimiento'];?>" <?=$disabled?>>
                                        </div>
												...
												<?php
                                                    ...
                                                    while($result = mysqli_fetch_array($consulta, MYSQLI_BOTH)){
                                                        ...
                                                        try{
                                                            ...
                                                            FROM comercial_relacion_productos WHERE czpp_producto='".$result[0]."' AND czpp_tipo=3 AND czpp_cotizacion='".$resultadoD['remi_id']."'");
                                                        } catch (Exception $e) {
                                                            include(RUTA_PROYECTO."includes/error-catch-to-report.php");
                                                        }
                                        ...
                                        ?>
                                            <div class="form-group col-md-11">
                                                ...
                                                <div class="input-group">
                                                    <textarea name="notas" rows="3" cols="80" style="width: 80%" placeholder="Observaciones..." <?=$readonly?>><?=$resultadoD['remi_observaciones'];?></textarea>
                                                </div>
                                            </div>
                                        <?php }else{ ?>
                                            <div class="form-group col-md-10">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        try{
                                            ...
                                            INNER JOIN comercial_relacion_productos ON czpp_producto=cprod_id AND czpp_cotizacion='".$_GET["id"]."' AND czpp_tipo=3 ORDER BY czpp_orden");
                                        } catch (Exception $e) {
                                            include(RUTA_PROYECTO."includes/error-catch-to-report.php");
                                        }
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="bd_delete/remisiones-productos-eliminar.php?idItem=<?=$prod['czpp_id'];?>&id=<?=$_GET["id"];?>" onClick="if(!confirm('Desea eliminar este registro?')){return false;}"><i class="icon-trash"></i></a>
