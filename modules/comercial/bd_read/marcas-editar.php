...
try {
	$resultadoD = SubCategorias::Select(["cmar_id" => $_GET["id"]])->fetch(PDO::FETCH_ASSOC);
} catch (Exception $e) {
					...
					<div class="row">
						...
						<div class="col-md-12">
							...
							<div class="card card-success">
								...
								<div class="col-md-12">
									<!-- form start -->
									<form class="form-horizontal" method="post" action="../bd_update/marcas-actualizar.php" enctype="multipart/form-data">
										...
										<div class="form-group col-md-6">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" multiple="multiple" style="width: 100%;" name="categorias[]">
												<option value=""></option>
												<?php
												$predicadoCat = [];
												if ($_SESSION["datosUsuarioActual"]['usr_tipo'] != DEV) {
													$predicadoCat = ['ccat_id_empresa' => $_SESSION["idEmpresa"]];
												}
												$categorias = Categorias::Select($predicadoCat)->fetchAll(PDO::FETCH_ASSOC);
												foreach ($categorias as $resOp) {

													$nombreEmpresa = '';
													if ($_SESSION["datosUsuarioActual"]['usr_tipo'] == DEV) {
														$empresa = Clientes_Admin::Select([ 'cliAdmi_id' => $resOp['ccat_id_empresa'] ])->fetch(PDO::FETCH_ASSOC);
														$nombreEmpresa = "[" . $empresa['cliAdmi_nombre'] . "]";
													}
													$selected = Sub_Categorias::numRows(
														[
															'subca_marca' => $resultadoD['cmar_id'],
															'subca_cate' => $resOp['ccat_id'],
															'subca_prin' => NO
														]) ? "selected" : "";
												?>
													<option value="<?= $resOp['ccat_id']; ?>" <?= $selected?>><?= $resOp['ccat_nombre'] . $nombreEmpresa; ?></option>
												<?php } ?>
											</select>
										</div>
										<div class="form-group col-md-6" style="display:block;" id="groupMenu">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="menu" id="menu" onchange="ocultarMasJoyas(this)">
												<option value=""></option>
												<option value="1" <?php if ($resultadoD['cmar_menu'] == 1) {
																		echo "selected";
																	} ?>>SI</option>
												<option value="0" <?php if ($resultadoD['cmar_menu'] == 0) {
																		echo "selected";
																	} ?>>NO</option>
											</select>
										</div>
										<div class="form-group col-md-6" style="display:block;" id="groupMasJoyas">
											...
											<select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="masJoyas" id="masJoyas">
												<option value=""></option>
												<option value="1" <?php if ($resultadoD['cmar_mas_productos'] == 1) {
																		echo "selected";
																	} ?>>SI</option>
												<option value="0" <?php if ($resultadoD['cmar_mas_productos'] == 0) {
																		echo "selected";
																	} ?>>NO</option>
											</select>
										</div>
