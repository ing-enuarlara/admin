            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="sliders-agregar.php" class="btn btn-primary"><i class="fas fa-solid fa-plus"></i> Agregar</a>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                $where="";
                                if($_SESSION["datosUsuarioActual"]['usr_tipo']!=DEV){
                                    $where= "AND slp_id_empresa='".$_SESSION["idEmpresa"]."'";
                                }
                                try{
                                    $sliders = $conexionBdPaginaWeb->query("SELECT slp.*, cliAdmi_nombre FROM sliders_paginas slp
                                    INNER JOIN " . BDADMIN . ".clientes_admin ON cliAdmi_id=slp_id_empresa 
                                    WHERE slp_id=slp_id $where");
                                } catch (Exception $e) {
                                    include(RUTA_PROYECTO."includes/error-catch-to-report.php");
                                }
                                $num=1;
                                while($result = mysqli_fetch_array($sliders, MYSQLI_BOTH)){

                                    $estado="SI";
                                    $color="green";
                                    if($result['slp_activo'] != SI){
                                        $estado="NO";
                                        $color="red";
                                    }

                                    $rutaFoto = "";
                                    if (!empty($result['slp_imagen'])) {
                                        switch ($result['slp_tipo_img']) {
                                            case TIPO_IMG:
                                                if (file_exists(RUTA_PROYECTO . "files/slider/".$result['slp_imagen'])) {
                                                    $rutaFoto = REDIRECT_ROUTE . "files/slider/".$result['slp_imagen'];
                                                }
                                                break;
                                            case TIPO_URL:
                                                $rutaFoto = $result['slp_imagen'];
                                                break;
                                        }
                                    }

                                    $texto = htmlspecialchars(strip_tags($result['slp_text']));
                                ?>
                                <tr>
                                    <td><?=$num;?></td>
                                    <td align="center">
                                    ...
                                    <td><?=strip_tags($result['slp_title']);?></td>
                                    <td title="<?=$texto;?>"
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                <a class="dropdown-item" href="sliders-editar.php?id=<?=$result[0];?>" data-toggle="tooltip">Editar</a>
                                                <!--<div class="dropdown-divider"></div>-->
                                                <a class="dropdown-item" href="../bd_delete/sliders-eliminar.php?id=<?=$result[0];?>" onClick="if(!confirm('Este registro se eliminará del sistema, Desea continuar bajo su responsabilidad?')){return false;}" data-toggle="tooltip">Eliminar</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
