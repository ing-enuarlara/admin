...
try {
  $consulta = $conexionBdPaginaWeb->query("SELECT * FROM sliders_paginas WHERE slp_id='" . $_GET["id"] . "'");
} catch (Exception $e) {
  include(RUTA_PROYECTO . "includes/error-catch-to-report.php");
}
$resultadoD = mysqli_fetch_array($consulta, MYSQLI_BOTH);

$rutaFoto = !empty($resultadoD['slp_tipo_img']) ? ($resultadoD['slp_tipo_img'] == TIPO_IMG ? REDIRECT_ROUTE . "files/slider/" . $resultadoD['slp_imagen'] : $resultadoD['slp_imagen']) : "";
?>
          ...
          <div class="row mb-2">
            <div class="col-sm-12">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?= REDIRECT_ROUTE ?>modules/pagina-web/bd_read/sliders.php">Ver Sliders & Banners</a></li>
          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="card card-success">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="post" action="../bd_update/sliders-actualizar.php" enctype="multipart/form-data">
                  <input type="hidden" name="id" value="<?= $_GET["id"]; ?>">
                  <div class="card-body">
                    <div class="col-md-6" style="margin-top: 10px;">
                      <div class="filtr-item col-md-12" data-category="1" data-sort="Foto Sliders & Banners">
                        <img src="<?= $rutaFoto ?>" class="img-fluid mb-2" alt="Foto Sliders & Banners" id="imgSlider" style="margin-left: auto; margin-right: auto; display: flex; flex-wrap: wrap; width: 200px; border-radius: 50%; max-width: 150px;" />
                      </div>
                    </div>
                    ...
                    <div class="form-group col-md-6">
                      ...
                      <select data-placeholder="Escoja una opción" class="form-control select2" onchange="cargarImagen(this)" style="width: 100%;" name="tipoImg" id="tipoImg">
                        <option value=""></option>
                          <option value="<?= TIPO_IMG ?>" <?=$resultadoD['slp_tipo_img'] == TIPO_IMG ? "selected" : "";?> >Imagen</option>
                          <option value="<?= TIPO_URL ?>" <?=$resultadoD['slp_tipo_img'] == TIPO_URL ? "selected" : "";?> >Url</option>
                      </select>
                    </div>

                    <div class="form-group col-md-6" id="tipoFile" style="display:none;">
                      <label for="customFile">Imagen del Slider:</label>
                      <div class="custom-file">
                        <input type="file" class="custom-file-input" id="customFile" name="imgSlider" onchange="cambiarImagen()">
                    ...
                    <div class="form-group col-md-6" id="tipoUrl" style="display:none;">
                      ...
                      <input type="text" class="form-control" placeholder="Url de la Imagen" value="<?= $resultadoD['slp_tipo_img'] == TIPO_URL ? $rutaFoto : "" ?>" name="urlImg" id="urlImg">
                    </div>

                    <hr>

                    <div class="form-group col-md-6">
                      <label for="exampleInputEmail1">Titulo Slider:</label>
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Titulo Slider" name="titulo" value="<?= $resultadoD['slp_title'] ?>" required>
                    </div>

                    <div class="form-group col-md-6">
                      <label for="contenido">Texto:</label>
                      <textarea name="contenido" id="contenido"><?= $resultadoD['slp_text'] ?></textarea>
                    </div>

                    <div class="form-group col-md-6">
                      <label>Activo?</label>
                      <select data-placeholder="Escoja una opción" class="form-control select2" style="width: 100%;" name="activo">
                        <option value=""></option>
                        <option value="<?= SI ?>" <?=$resultadoD['slp_activo'] == SI ? "selected" : "";?> >SI</option>
                        <option value="<?= NO ?>" <?=$resultadoD['slp_activo'] == NO ? "selected" : "";?> >NO</option>
                      </select>
                    </div>
                  </div>
                  <!-- /.card-body -->
