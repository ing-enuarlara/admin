...
<head>
	<meta charset="utf-8">
	<title>Remisión #<?= date("dmy", strtotime($resultado['remi_fecha_propuesta']))."-".$resultado['remi_id']; ?></title>
		...
		<div id="contenedor">
			<div class="p-2 m-2" id="encabezado">
				<div class="row mb-2">
					<div class="col-5">
						<div class="card-body">
							<h5 class="card-title" style="color: #a30000; font-weight:bold;"><?= strtoupper($_SESSION["configuracion"]['conf_empresa']); ?></h5>
							<p class="card-text">
						...
						<div class="card-body">
							<h5 class="card-title" style="background-color: #a30000; color:white; font-weight:bold;">
								CLIENTE
							</h5>
							<h6 class="card-subtitle mb-1 text-muted" style="color: #a30000; font-weight:bold;">
					...
					<div class="col-5 text-right">
						<div class="card-body">
							<h5 class="card-title" style="color: #a30000; font-weight:bold;">REMISIÓN #<?= date("dmy", strtotime($resultado['remi_fecha_propuesta']))."-".$resultado['remi_id']; ?></h5>
							<p class="card-text">
			...
			<div style="margin: 10px; font-size: 12px;" align="center">
				<table width="100%" border="0" rules="groups" class="border border-dark rounded p-2">
					<thead align="center">
						<tr style="background-color: #a30000; height: 50px; color: white;">
							<th>Nº</th>
							<th>&nbsp;</th>
							<th>Productos</th>
							<th>Cant.</th>
						...
						?>
							<tr style="height: 30px; background-color: <?= $fondo; ?>;" class="p-2">
								...
								<td class="pl-2">
									...
									<span style="font-size: 9px; color: #a30000;"><?= $prod['cprod_detalles']; ?></span><br>
									<span style="font-size: 9px; color: #a30000;"><?= $prod['czpp_observacion']; ?></span>
								</td>
								<td align="center" class="alinear"><?= $prod['czpp_cantidad']; ?></td>
								<td align="center" class="alinear"><?= $simbolosMonedas[$resultado['remi_moneda']]; ?><?= number_format($prod['czpp_valor'], 0, ",", "."); ?></td>
								...
								<td align="right" class="alinear pr-2"><?= $simbolosMonedas[$resultado['remi_moneda']]; ?><?= number_format($valorTotal, 0, ",", "."); ?></td>
							</tr>
						<?php
							$no++;
						}
					...
					<tfoot>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td colspan="3" rowspan="4" class="alinear p-1">
								<?php if (!empty($resultado['remi_observaciones'])) { ?>
							...
							<td style="text-align: right;" colspan="3">SUBTOTAL <?= $simbolosMonedas[$resultado['remi_moneda']]; ?></td>
							<td align="right" colspan="2" class="pr-2"><?php if (!empty($subtotal)) echo number_format($subtotal, 0, ",", "."); ?></td>
						</tr>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td style="text-align: right;" colspan="3">DESCUENTO <?= $simbolosMonedas[$resultado['remi_moneda']]; ?></td>
							<td align="right" colspan="2" class="pr-2"><?php if (!empty($totalDescuento)) echo number_format($totalDescuento, 0, ",", "."); ?></td>
						</tr>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td style="text-align: right;" colspan="3">IVA <?= $simbolosMonedas[$resultado['remi_moneda']]; ?></td>
							<td align="right" colspan="2" class="pr-2"><?php if (!empty($totalIva)) echo number_format($totalIva, 0, ",", "."); ?></td>
						</tr>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td style="text-align: right; background-color: #a30000; color:white;" colspan="3">TOTAL NETO <?= $simbolosMonedas[$resultado['remi_moneda']]; ?></td>
							<td align="right" style="background-color: #a30000; color:white;" colspan="2" class="pr-2"><?php if (!empty($total)) echo number_format($total, 0, ",", "."); ?></td>
						</tr>
					</tfoot>
				</table>
			</div>
			<div class="m-2" style="font-size: 13px;" id="pie">
				<?php if(!empty($_SESSION["configuracion"]['conf_observaciones_remisiones'])){ ?>
					<div class="border border-dark rounded">
						<h5 class="card-title p-2" style="background-color: #a30000; color:white; font-weight:bold;">TÉRMINOS Y CONDICIONES</h5>
						<p class="card-text p-2">
							<?= $_SESSION["configuracion"]['conf_observaciones_remisiones']; ?>
						</p>
					</div>
				<?php } ?>
