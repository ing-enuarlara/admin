...
<head>
	<meta charset="utf-8">
	<title>Cotización #<?= date("dmy", strtotime($resultado['cotiz_fecha_propuesta']))."-".$resultado['cotiz_id']; ?></title>
		...
		<div id="contenedor">
			<div class="p-2 m-2" id="encabezado">
				<div class="row mb-2">
					<div class="col-5">
						...
						<div class="card-body">
							<h5 class="card-title" style="background-color: green; color:white; font-weight:bold;">
								CLIENTE
							</h5>
							<h6 class="card-subtitle mb-1 text-muted" style="color: green; font-weight:bold;">
					...
					<div class="col-5 text-right">
						<div class="card-body">
							<h5 class="card-title" style="color: green; font-weight:bold;">COTIZACIÓN #<?= date("dmy", strtotime($resultado['cotiz_fecha_propuesta']))."-".$resultado['cotiz_id']; ?></h5>
							<p class="card-text">
			...
			<div style="margin: 10px; font-size: 12px;" align="center">
				<table width="100%" border="0" rules="groups" class="border border-dark rounded p-2">
					<thead align="center">
						<tr style="background-color: green; height: 50px; color: white;">
							<th>Nº</th>
							<th>&nbsp;</th>
							<th>Productos</th>
							<th>Cant.</th>
						...
						?>
							<tr style="height: 30px; background-color: <?= $fondo; ?>;" class="p-2">
								...
								<td class="pl-2">
									...
									<span style="font-size: 9px; color: green;"><?= $prod['cprod_detalles']; ?></span><br>
									<span style="font-size: 9px; color: green;"><?= $prod['czpp_observacion']; ?></span>
								</td>
								...
								<td align="right" class="alinear pr-2"><?= $simbolosMonedas[$resultado['cotiz_moneda']]; ?><?= number_format($valorTotal, 0, ",", "."); ?></td>
							</tr>
						<?php
							$no++;
						}
					...
					<tfoot>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td colspan="3" rowspan="5" class="alinear p-1">
								<?php if (!empty($resultado['cotiz_observaciones'])) { ?>
									<div style="height: 95px; display:block;" class="border border-dark rounded p-1">
										<b>OBSERVACIONES:</b><br>
										<span style="font-weight: normal;">
											<?= $resultado['cotiz_observaciones']; ?>
										</span>
									</div>
								<?php } ?>
							</td>
						...
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							...
							<td align="right" colspan="2" class="pr-2"><?php if (!empty($envio)) echo number_format(floatval($envio), 0, ",", "."); ?></td>
						</tr>
						<tr style="font-weight: bold; font-size: 13px; height: 20px;">
							<td style="text-align: right; background-color: green; color:white;" colspan="3">TOTAL NETO <?= $simbolosMonedas[$resultado['cotiz_moneda']]; ?></td>
							<td align="right" style="background-color: green; color:white;" colspan="2" class="pr-2"><?php if (!empty($total)) echo number_format($total, 0, ",", "."); ?></td>
						</tr>
					</tfoot>
				</table>
			</div>
